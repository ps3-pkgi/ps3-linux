				 ...
				 struct gelic_descr *start_descr, int descr_count)
{
	...
	for (index = 0, descr = start_descr; index < descr_count; index++,
		...
		if (dma_mapping_error(dev, descr->link.cpu_addr)) {
				  ...
				  struct gelic_descr *descr)
{
	...
	descr->hw_regs.payload.size = cpu_to_be32(GELIC_NET_MAX_MTU);
	...
	descr->hw_regs.payload.dev_addr = cpu_to_be32(dma_map_single(dev, napi_buff,
		GELIC_NET_MAX_MTU, DMA_FROM_DEVICE));

	if (dma_mapping_error(dev, descr->hw_regs.payload.dev_addr)) {
		skb_free_frag(napi_buff);
		descr->skb = NULL;
		descr->hw_regs.payload.dev_addr = 0;
		descr->hw_regs.payload.size = 0;
				 ...
				 struct gelic_descr *descr)
{
	...
	if (gelic_descr_get_status(descr) == GELIC_DESCR_DMA_CARDOWNED) {
		...
		if (status) {
			...
			dev_info(dev, "lv1_net_start_txdma failed," \
				 "status=%d\n", status);
		}
	}
	return status;
...
int gelic_net_setup_netdev(struct net_device *netdev, struct gelic_card *card)
{
	...
	if (status) {
		dev_err(dev, "%s:Couldn't register %s %d\n",
			__func__, netdev->name, status);
		return status;
	}
	dev_info(dev, "%s: MAC addr %pM\n",
...
static void gelic_card_get_vlan_info(struct gelic_card *card)
{
	...
	for (i = 0; i < ARRAY_SIZE(vlan_id_ix); i++) {
		...
		dev_dbg(dev, "vlan_id[%d] tx=%02x rx=%02x\n",
...
static int ps3_gelic_driver_probe(struct ps3_system_bus_device *sb_dev)
{
	...
	if (result) {
		dev_info(dev, "%s:request_irq failed (%d)\n",
			__func__, result);
	...
	dev_dbg(dev, "descr rx %p, tx %p, size %#lx, num %#x\n",
	...
	if (result) {
		dev_dbg(dev, "%s: setup_netdev failed %d\n",
			__func__, result);
